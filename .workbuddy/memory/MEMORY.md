...
- **预计/实际用时**（2026-06-24）：执行图 title 旁淡色(C.LABEL)显示，`_steps_title(running)` 生成，格式 `HH:MM:SS`（`_fmt_hms` 工具），标签后带冒号（`预计: HH:MM:SS`、`实际: HH:MM:SS`）。预计=`_estimate_loop_duration`（所有 nav 步骤 delay 之和），实际=`time.monotonic()-_loop_start_time`（每轮重置）。非运行只显示预计。
